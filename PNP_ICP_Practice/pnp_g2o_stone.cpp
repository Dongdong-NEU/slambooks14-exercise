...
int main ( int argc, char** argv )
{
    ...
    // 建立3D点
    Mat d1 = imread ( argv[3], CV_LOAD_IMAGE_UNCHANGED );// 深度图为16位无符号数，单通道图像 CV_LOAD_IMAGE_UNCHANGED表示读取原始图像
...
/*    Block::LinearSolverType* linearSolver = new g2o::LinearSolverCSparse<Block::PoseMatrixType>(); // 线性方程求解器
    ...
    for ( const Point2f &p:points_2d )//每个点都会与位姿节点链接 所以就有那么多的边
    {
        ...
        //下行转化 要用动态类型转换
        //将2元边连接上顶点
        edge->setVertex ( 0, dynamic_cast<g2o::VertexSBAPointXYZ*> ( optimizer.vertex ( index ) ) );//空间点类型指针

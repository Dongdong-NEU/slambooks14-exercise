...
#include <Eigen/Core> 
#include <Eigen/Dense> // 当有#include <opencv2/core/eigen.hpp>  #include <Eigen/Dense> 要放前面;
...
int main(int argc, char **argv) {
  ...
  Eigen::Matrix<double,3,1> eigen_v;
  cv::cv2eigen(R, eigen_R);
  cv::cv2eigen(t, eigen_v);
  Sophus::SE3d p1(eigen_R, eigen_v);
  ...
  // 调用RANSAC来排除一些外点(误匹配的点);false:是否使用自己提供的r t作为SOLVEPNP_ITERATIVE算法迭代初始值,false是否;
  // 100 是迭代次数, 2 是表示RANSAC算法的阈值,用于判断内外点;
  cv::Mat r2;
  cv::Mat tt;
  // solvePnP(bad_pts_3d, bad_pts_2d, K, Mat(), r2, tt, false); // 调用OpenCV 的 PnP 求解，可选择EPNP，DLS等方法
  solvePnPRansac(bad_pts_3d, bad_pts_2d, K, Mat(), r2, tt, false, 100,0.7); 
  cv::Mat R2; 
  ...
  Eigen::Matrix<double,3,3> eigen_R2;
  Eigen::Matrix<double,3,1> eigen_v2;
  cv::cv2eigen(R2, eigen_R2);
  cv::cv2eigen(tt, eigen_v2);
  Sophus::SE3d p2(eigen_R2, eigen_v2);

  double err_ = (p1.inverse() * p2).log().norm();
  std::cout << "err_ is :" << err_ << std::endl;
  waitKey(0);
